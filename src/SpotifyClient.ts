...
export class SpotifyClient {
  ...
  // A token passed manually by the developer (e.g., from frontend user login)
  ...
  constructor(options: SpotifyOptions) {
    if (!options.clientId || !options.clientSecret) {
      throw new SpotifyError('clientId and clientSecret are required');
    }
  ...
  /**
   * Sets the User Token generated by the OAuth login flow to access private playlists, player, etc.
   */
  ...
  /**
   * Fetches a system access token (Client Credentials) to access public data.
   */
  private async getClientToken(): Promise<string> {
    ...
    if (!response.ok) {
      ...
      throw new SpotifyError(
        errorData.error_description || 'Spotify Authentication (Client Credentials) failed',
        response.status
      );
    }
  ...
  public async rawRequest(endpoint: string, fetchOptions: RequestInit): Promise<Response> {
    ...
    const executeRequest = async () => {
      try {
        ...
        if (error.name === 'AbortError') throw new SpotifyError('The request timed out.');
        if (error instanceof SpotifyError) throw error;
        throw new SpotifyError(`Request failed: ${error.message}`);
      }
    };
  ...
  /**
   * Standard helper for cached GET requests.
   */
